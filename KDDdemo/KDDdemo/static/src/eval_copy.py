...
def aggregate_ranking(sim, cap, word_cap, upper, lower, topic, id2topic, idx2word, pretrain, out_file):
    # sim_cand = sim[:100]
    # cap_cand = cap[:100]
...
if __name__ == "__main__":
    ...
    if args.pretrain == '0':
        ...
        #word_emb, vocabulary, vocabulary_inv = get_emb(vec_file=args.emb)
        #word_emb, vocabulary, vocabulary_inv = get_emb(vec_file=os.path.join(args.dataset, args.emb + '.txt'))
        '''embedding = make_emb_mat(word_emb, vocabulary_inv)
        pickle.dump(vocabulary, open("word2id.p", 'wb'))
        pickle.dump(vocabulary_inv, open("id2word.p", 'wb'))
        pickle.dump(embedding, open("embedding.p", 'wb'))
        read_file(os.path.join(args.dataset, 'phrase_text_train.txt'), vocabulary, 'train_text.p')
        read_file(os.path.join(args.dataset, 'phrase_text_test.txt'), vocabulary, 'test_text.p')'''
        ...
        word_cap_dict = get_cap(vec_file=os.path.join(args.dataset, args.emb + '_cap.txt'))
    ...
    for topic in topic_emb:
        ...
        if args.pretrain == '1':
            ...
            cap_ranking, word_cap1  = rank_cap(word_cap_dict, vocabulary_inv, vocabulary[topic])
        for i in range(0,4):
            aggregate_ranking(sim_ranking, cap_ranking, word_cap1, upper, lower, topic, id2topic, vocabulary_inv, args.pretrain, os.path.join(args.dataset, args.out_file))
            upper += 0.25
            lower += 0.25
