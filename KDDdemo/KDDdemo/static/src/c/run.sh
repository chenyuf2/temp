...
then
		...
		# ./word2vec -train ./${dataset}/${text_file} -output ./${dataset}/${emb_file}_w.txt -kappa ./${dataset}/${emb_file}_cap.txt -topic ./${dataset}/topics_${topic_file}.txt -topic_output ./${dataset}/${emb_file}_t.txt -reg_lambda 10 -cbow 0 -size 100 -global_lambda 1.5 -window 5 -negative 5 -sample 1e-3 -min-count 5 -threads 20 -binary 0 -iter 10 -pretrain 5 -rank_product 0 -gen_vocab 1 -load_emb 0
  #  python get_pretrained.py
		./word2vec -train ./${dataset}/${text_file} -output ./${dataset}/emb_${topic_file}_w.txt -kappa ./${dataset}/emb_${topic_file}_cap.txt -topic ./${dataset}/topics_${topic_file}.txt -topic_output ./${dataset}/emb_${topic_file}_t.txt -reg_lambda 10 -cbow 0 -size 100 -global_lambda 1.5 -window 5 -negative 5 -sample 1e-3 -min-count 5 -threads 20 -binary 0 -iter 5  -pretrain 1 -rank_product 0 -gen_vocab 0 -load_emb 1
