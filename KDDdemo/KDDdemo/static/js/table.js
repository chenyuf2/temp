...
function loadJSON3(callback) {
            var xobj = new XMLHttpRequest();
            xobj.overrideMimeType("text/plain");
            xobj.open('GET', "static/src/papers/results_dm_fixed.txt", true);
            xobj.onreadystatechange = function () {
              if (xobj.readyState == 4 && xobj.status == "200") {
                callback(xobj.responseText);
              }
...
function loadJSON4(callback) {
            var xobj = new XMLHttpRequest();
            xobj.overrideMimeType("text/plain");
            xobj.open('GET', "static/src/papers/results2_dm_fixed.txt", true);
            xobj.onreadystatechange = function () {
              if (xobj.readyState == 4 && xobj.status == "200") {
                callback(xobj.responseText);
              }
...
var table3_dict = {};
for (var i = 0; i < table3_array.length-1; i++) {
  if (table3_array[i] == "") {
    continue;
  }
  else {
    if (table3_array[i].includes(":")) {
      table3_dict[table3_array[i].substr(0,table3_array[i].length - 1)]= table3_array[i+1].split(" ");
    }
  }
}
// console.log(table3_dict);
//For table2
...
var table4_array = table4.split("\n");
var table4_dict = {};
for (var i = 0; i < table4_array.length - 1; i++) {
  if (table4_array[i] == "") {
    continue;
  }
  else {
    if (table4_array[i][table4_array[i].length - 1] == ":") {
      var key = table4_array[i].substr(0,table4_array[i].length - 1);
      if (!(key in table4_dict)) {
        table4_dict[key] = [];
        table4_dict[key].push(table4_array[i+1]);
      }
      else {
      table4_dict[key].push(table4_array[i+1]);
      }
...
for (var key in table4_dict) {
  for (var i = 0; i < table4_dict[key].length; i++) {
    table4_dict[key][i] = table4_dict[key][i].split(":\t");
  }
}
console.log(table1_dict);
console.log(table3_dict);
var key_array = Object.keys(table1_dict);


if (key_array.length == 3 && key_array.includes("machine_learning")==true && key_array.includes("data_mining") && key_array.includes("natural_language_processing")) {
  table1_dict = table3_dict;
  table2_dict = table4_dict;
}
